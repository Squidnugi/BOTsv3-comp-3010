...
**Recovery:** A "Lessons Learned" review must address the underlying policy breaches. Recommendations include enforcing the Principle of Least Privilege to ensure single-user misconfigurations cannot expose the organisation's entire attack surface.
...
This investigation answers BOTSv3 200-level questions 201-208: IAM user count, MFA status, web server identification, S3 bucket name, uploaded files, and endpoint attribution. Answers are presented chronologically through the investigation stages below.
